<script lang="ts">
	import type { RouteSubMenu, RouteItemType } from '$lib/type.js';
	import _ from 'lodash';
	const dataRoute: RouteSubMenu[] = [
		{
			hasSubMenu: true,
			state: 'Home', // các route sẽ nhớ và active lại khi f5: [state,path+name]
			name: 'Vật tư, hóa chất ',
			path: '/home',
			icon: `<svg class="w-5 h-5 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 22 21">
	...
	>
					...
					{#each dataRoute as routeItem, i (i)}
						{#if routeItem.hasSubMenu}
							<li>
								...
								>
									{@html routeItem.icon}
									<span class="flex-1 ml-3 text-left whitespace-nowrap">{routeItem.name}</span>
									<svg
										class="w-6 h-6"
										fill="currentColor"
										viewBox="0 0 20 20"
								...
								<ul id="dropdown-layouts" class="py-2 space-y-2">
									{#each routeItem?.sub as dataItemLevel2, i (i)}
										<li>
											<a
												href={`${dataItemLevel2.path}`}
						...
						{:else}
							<li>
								...
								>
									{@html routeItem.icon}
									<span class="ml-3">{routeItem.name}</span>
								</a>
							</li>
						{/if}
					{/each}
		...
		>
			...
			<div
				...
				style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate(184px, 2396px);"
